...
=======
 ...
 -i --inplace    Tidy given file(s) so that original file(s) are overwritten
                 ...
                 Examples:
                   python -m robot.tidy --inplace smoke_test.robot
...
Examples:
  ...
  python -m robot.tidy --inplace smoke_test.robot
  python -m robot.tidy --recursive path/to/smoke_test
...
especially useful if there is a need to convert smoke_test in deprecated HTML
...
Examples:
  python -m robot.tidy smoke_test.html smoke_test.robot
  python -m robot.tidy --format robot --inplace smoke_test.html
  python -m robot.tidy --format robot --recursive path/to/smoke_test
...
def tidy_cli(arguments):
    ...
    Example::
        ...
        tidy_cli(['--format', 'txt', 'smoke_test.html'])
    """
