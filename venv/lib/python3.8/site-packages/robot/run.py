...
"""Module implementing the command line entry point for executing smoke_test.
...
=======
 ...
 -i --include tag *       Select test cases to run by tag. Similarly as name
                          ...
                          Examples: --include foo --include bar*
                                    --include fooANDbar*
 -e --exclude tag *       Select test cases not to run by tag. These smoke_test are
                          not run even if included with --include. Tags are
                          matched using the rules explained with --include.
 -R --rerunfailed output  Select failed smoke_test from an earlier output file to be
                          re-executed. Equivalent to selecting same smoke_test
 ...
 -d --outputdir dir       Where to create output files. The default is the
                          directory where smoke_test are run from and the given path
 ...
 -x --xunit file          xUnit compatible result file. Not created unless this
                          option is specified.
    --xunitskipnoncritical  Mark non-critical smoke_test on xUnit output as skipped.
 ...
 -T --timestampoutputs    When this option is used, timestamp in a format
    ...
    --maxerrorlines lines  Maximum number of error message lines to show in
                          report when smoke_test fail. Default is 40, minimum is 10
 ...
 -L --loglevel level      Threshold level for logging. Available levels: TRACE,
    ...
    --tagstatcombine tags:name *  Create combined statistics based on tags.
                          ...
                          and matching smoke_test into `Test Details`. If optional
    ...
    --runemptysuite       Executes smoke_test also if the top level test suite is
                          empty. Useful e.g. with --include/--exclude when it
                          is not an error that no test matches the condition.
    --dryrun              Verifies test data and runs smoke_test so that library
 ...
 -X --exitonfailure       Stops test execution if any critical test fails.
    ...
    --skipteardownonexit  Causes teardowns to be skipped if test execution is
                          stopped prematurely.
    --randomize all|suites|smoke_test|none  Randomizes the test execution order.
                          all:    randomizes both suites and smoke_test
                          suites: randomizes suites
                          smoke_test:  randomizes smoke_test
                          none:   no randomization (default)
                          Use syntax `VALUE:SEED` to give a custom random seed.
                          The seed must be an integer.
                          Examples: --randomize all
                                    --randomize smoke_test:1234
    ...
    --console type        How to report execution on the console.
                          ...
                          dotted:   only show `.` for passed test, `f` for
                                    failed non-critical smoke_test, and `F` for
                                    failed critical smoke_test
...
ROBOT_SYSLOG_FILE         Path to a file where Robot Framework writes internal
                          ...
                          smoke_test. Can be useful when debugging problems. If not
...
$ robot smoke_test.robot

# Using options.
$ robot --include smoke --name "Smoke Tests" path/to/smoke_test.robot

# Executing `robot` module using Python.
$ python -m robot path/to/smoke_test

# Running `robot` directory with Jython.
$ jython /opt/robot smoke_test.robot

# Executing multiple test case files and using case-insensitive long options.
$ robot --SuiteStatLevel 2 --Metadata Version:3 smoke_test/*.robot more/smoke_test.robot

# Setting default options and syslog file before running smoke_test.
...
def run_cli(arguments=None, exit=True):
    """Command line execution entry point for running smoke_test.
    ...
    Entry point used when running smoke_test from the command line, but can also
    be used by custom scripts that execute smoke_test. Especially useful if the
    ...
    Example::
        ...
        # Run smoke_test and return the return code.
        rc = run_cli(['--name', 'Example', 'smoke_test.robot'], exit=False)

        # Run smoke_test and exit to the system automatically.
        run_cli(['--name', 'Example', 'smoke_test.robot'])
...
def run(*tests, **options):
    """Programmatic entry point for running smoke_test.
    ...
    line options support. For example, ``run('smoke_test', listener=MyListener())``.
    ...
    Zero means that smoke_test were executed and no critical test failed, values up
    to 250 denote the number of failed critical smoke_test, and values between
    ...
    Example::
        ...
        run('path/to/smoke_test.robot')
        run('smoke_test.robot', include=['tag1', 'tag2'], splitlog=True)
        with open('stdout.txt', 'w') as stdout:
            run('t1.robot', 't2.robot', name='Example', log=None, stdout=stdout)

    Equivalent command line usage::

        robot path/to/smoke_test.robot
        robot --include tag1 --include tag2 --splitlog smoke_test.robot
