...
class TestCaseFile(_TestData):
    ...
    def _validate(self):
        if not self.testcase_table.is_started():
            raise NoTestsFound('File has no smoke_test or tasks.')
...
class ResourceFile(_TestData):
    ...
    def _table_is_allowed(self, table):
        if table is self.testcase_table:
            raise DataError("Resource file '%s' cannot contain smoke_test or "
                            "tasks." % self.source)
        return True
    ...
    def _table_is_allowed(self, table):
        if table is self.testcase_table:
            LOGGER.error("Test suite initialization file in '%s' cannot "
                         "contain smoke_test or tasks." % self.source)
            return False
        return True
...
class TestCaseTable(_Table):
    ...
    def _validate_mode(self, name1, name2):
        ...
        if tasks1 is not tasks2:
            raise DataError('One file cannot have both smoke_test and tasks.')
